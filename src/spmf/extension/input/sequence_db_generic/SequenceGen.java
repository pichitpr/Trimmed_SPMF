...
*
* @see SequenceDatabaseGen
 * @author Philipe-Fournier-Viger
 */
public class SequenceGen<T extends Comparable<T>> {
	 ...
	 */
	public SequenceGen<T> cloneSequenceMinusItems(Map<T, Set<Integer>> mapSequenceID, double relativeMinSup) {
		// create a new sequence
		SequenceGen<T> sequence = new SequenceGen<T>(getId());
	 ...
	 * @return a copy of this sequence except that item(s) with a support lower than minsup have been excluded.
	 */
	public SequenceGen<T> cloneSequenceMinusItems(double relativeMinSup, Map<T, Set<SequenceGen<T>>> mapSequenceID) {
		// create a new sequence
		SequenceGen<T> sequence = new SequenceGen<T>(getId());
		// for each  itemset in the original sequence
		for(List<T> itemset : itemsets){
			// call a method to copy this itemset
			List<T> newItemset = cloneItemsetMinusItems(relativeMinSup, itemset, mapSequenceID);
			// add the copy to the new sequence
			if(newItemset.size() !=0){ 
				sequence.addItemset(newItemset);
			} 
	 ...
	 */
	public List<T> cloneItemsetMinusItems(double relativeMinsup, List<T> itemset,Map<T, Set<SequenceGen<T>>> mapSequenceID) {
	 ...
	 * @return a copy of this itemset except that item(s) with a support lower than minsup have been excluded.
	 */
	public List<T> cloneItemsetMinusItems(List<T> itemset,Map<T, Set<Integer>> mapSequenceID, double minSupportAbsolute) {
		// create a new itemset
		List<T> newItemset = new ArrayList<T>();
		// for each item of the original itemset
		for(T item : itemset){
