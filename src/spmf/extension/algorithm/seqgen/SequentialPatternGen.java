 ...
 */
public class SequentialPatternGen<T extends Comparable<T>> 
	implements Comparable<SequentialPatternGen<T>>{
	
	// the list of itemsets
	private final List<ItemsetGen<T>> itemsets;
//	private int id; 
	 ...
	 */
	public SequentialPatternGen(){
		itemsets = new ArrayList<ItemsetGen<T>>();
	}
	
	public SequentialPatternGen(ItemsetGen<T> itemset, Set<Integer> sequencesIds){
		itemsets = new ArrayList<ItemsetGen<T>>();
		this.itemsets.add(itemset);
		this.sequencesIds = sequencesIds;
	}
	
	public SequentialPatternGen(List<ItemsetGen<T>> itemsets, Set<Integer> sequencesIds){
	 ...
	 */
	public void addItemset(ItemsetGen<T> itemset) {
//		itemCount += itemset.size();
	 ...
	 * @return the copy.
	 */
	public SequentialPatternGen<T> cloneSequence(){
		// create a new empty sequential pattenr
		SequentialPatternGen<T> sequence = new SequentialPatternGen<T>();
		// for each itemset
		for(ItemsetGen<T> itemset : itemsets){
...
//		}
	 ...
	 */
	public List<ItemsetGen<T>> getItemsets() {
		return itemsets;
	}
	 ...
	 * @return the itemset
	 */
	public ItemsetGen<T> get(int index) {
	...
	@Override
	public int compareTo(SequentialPatternGen<T> o) {
			if(o == this){
				return 0;
			}
