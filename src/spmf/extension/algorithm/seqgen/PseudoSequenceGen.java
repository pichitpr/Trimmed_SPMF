 ...
 */
public class PseudoSequenceGen<T extends Comparable<T>> {

	// the corresponding sequence in the original database
	protected SequenceGen<T> sequence;
	 ...
	 */
	public SequenceGen<T> getOriginalSequence() {
		return sequence;
	}
	 ...
	 */
	protected PseudoSequenceGen(PseudoSequenceGen<T> sequence, int indexItemset, int indexItem){
	 ...
	 * @param indexItem the item where the pseudo-sequence should start in terms of the original sequence.
	 */
	protected  PseudoSequenceGen(SequenceGen<T> sequence, int indexItemset, int indexItem){
		// remember the original sequence
	 ...
	 * @return the item.
	 */
	public T getItemAtInItemsetAt(int indexItem, int indexItemset) {
	 ...
	 * @return the itemset
	 */
	public List<T> getItemset(int index) {
...
//				}
	 ...
	 */
	protected int indexOfBis(int indexItemset, T idItem) {
		// for each item in that itemset
		for(int i=0; i < getSizeOfItemsetAt(indexItemset); i++){
			// check if equals to the item that we search
			if(getItemAtInItemsetAt(i, indexItemset).compareTo(idItem) == 0){
				return i; // if equal, return the current position
			}else if(getItemAtInItemsetAt(i, indexItemset).compareTo(idItem) > 0){
				continue;
			}
		}
	 ...
	 * @return the position of the item or -1 if it is not found
	 */
	protected int indexOf(int sizeOfItemsetAti, int indexItemset, T idItem) {
		// for each item in that itemset
	...
	public boolean equals(Object obj) {
		PseudoSequenceGen<T> temp = (PseudoSequenceGen<T>) obj;
